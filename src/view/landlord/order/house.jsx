...
class orderComponent extends Component {
    ...
    updatestatus = data => { // done
        // [1,2,3] 全部false  - 服务端处理
        // [1] 这种看情况
        post('/landlord/order/updatestatus', { ...data }).then(res => {
    ...
    delete = id => {//刪 done
        post('/landlord/order/delete', { id: id }).then(res => {
    ...
    get = () => {//查 done
        ...
        post('/landlord/order/get', params).then(res => {
    ...
    editmodalhandleOk = (record) => {//确认
        ...
        switch (this.state.editmodal.type) {
            case "create":
                post('/landlord/order/create', record).then(res => {
                    message.success("新增成功")
                    this.get()
                })
                break;
            case "edit":
                post('/landlord/order/update', record).then(res => {
                    message.success("更新成功")
                    this.get()
                })
                break;
