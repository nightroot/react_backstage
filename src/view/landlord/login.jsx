    ...
    class loginComponent extends Component {
        ...
        render() {
            const sendSms = (values) => {
                // 发送验证码
                post('/landlord/sendsms', { mobile: this.state.mobile }).then(res => {
                    if (res.status) {
            ...
            const login = (values) => {
                // 登录
                post('/landlord/login', values).then(res => {
                    this.props.setlandlorduser(res)//保存登录状态-全局
                    this.props.history.push('/landlord/index')
                })
            };
            return (
