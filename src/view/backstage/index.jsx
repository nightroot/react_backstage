...
import { backstagemenu } from '@/routes/backstage'
...
class IndexComponent extends Component {
    ...
    componentDidMount() {
        // 挂载 - 检查状态
        this.setState({ userinfo: { ...this.props.backstageuser } })
    }
    ...
    handleUserMenuClick = (e) => {
        ...
        if (e.key === "logout") {
            post('/backstage/logout', {
                auth:this.props.backstageuser.token,
                mobile:this.props.backstageuser.mobile
            }).then((res => {
                this.props.setbackstageuser({})//注销
                message.success("注销成功")
                this.props.history.push('/backstage/login')
            }))
        }
                ...
                <Header style={{ color: 'white', zIndex: 1, width: '100%', padding: 0 }}>
                    <Row wrap={false} justify="space-between">
                        ...
                        <Image
                            ...
                            />
                            <Title style={{ color: 'white', display: 'inline-block', marginTop: 6 }} level={3}>安居房屋租赁后台管理系统</Title>
                        </Col>
                ...
                <Layout>
                    ...
                    >
                        <Menu theme="dark" defaultSelectedKeys={['1']} mode="inline">
                            {backstagemenu.map(parents => {
                                if (parents.isShow === false) { return {} }
                                if (parents.children === undefined) {
                                    return (
                                        <Menu.Item key={parents.path} onClick={p => this.props.history.push(`/backstage${p.key}`)}>
                                    ...
                                    return (
                                        <SubMenu key={parents.path} title={
                                            ...
                                            {parents.children.map((child) => {
                                                return (
                                                    <Menu.Item key={child.path} onClick={p => this.props.history.push(`/backstage${p.key}`)}>
