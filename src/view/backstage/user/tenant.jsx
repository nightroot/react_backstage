...
class tenantComponent extends Component {
    ...
    updatestatus = data => { // done
        // [1,2,3] 全部false  - 服务端处理
        // [1] 这种看情况
        post('/backstage/user/tenant/updateenable', { id_list: data }).then(res => {
    ...
    delete = id => {//刪 done
        post('/backstage/user/tenant/delete', { id: id }).then(res => {
    ...
    get = () => {//查 done
        ...
        post('/backstage/user/tenant/get', params).then(res => {
    ...
    editmodalhandleOk = (record) => {//确认
        ...
        switch (this.state.editmodal.type) {
            case "create":
                post('/backstage/user/tenant/create', upload).then(res => {
                    message.success("新增成功")
                    this.get()
                })
                break;
            case "edit":
                post('/backstage/user/tenant/update', upload).then(res => {
                    message.success("更新成功")
                    this.get()
                })
                break;
