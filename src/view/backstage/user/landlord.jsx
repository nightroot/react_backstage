...
class landlordComponent extends Component {
    ...
    updatestatus = data => { // done
        // [1,2,3] 全部false  - 服务端处理
        // [1] 这种看情况
        post('/backstage/user/landlord/updateenable', { id_list: data }).then(res => {
    ...
    delete = id => {//刪 done
        post('/backstage/user/landlord/delete', { id: id }).then(res => {
    ...
    get = () => {//查 done
        ...
        post('/backstage/user/landlord/get', params).then(res => {
    ...
    editmodalhandleOk = (record) => {//确认
        if (record.avatarimg !== "" && record.avatarimg !== undefined) {
            record.avatar = record.avatarimg.fileList.length !== 0 ? record.avatarimg.fileList[0].thumbUrl : record.avatar
        }
        if (record.collectionqrcodeimg !== "" && record.collectionqrcodeimg !== undefined) {
            record.collectionqrcode = record.collectionqrcodeimg.fileList.length !== 0 ? record.collectionqrcodeimg.fileList[0].thumbUrl : record.collectionqrcode
        }
        let upload = Object.assign(this.state.editmodal.data, record)
        delete upload.avatarimg
        delete upload.collectionqrcodeimg//清理无用参数

        switch (this.state.editmodal.type) {
            case "create":
                post('/backstage/user/landlord/create', upload).then(res => {
                    message.success("新增成功")
                    this.get()
                })
                break;
            case "edit":
                post('/backstage/user/landlord/update', upload).then(res => {
                    message.success("更新成功")
                    this.get()
                })
                break;
    ...
    render() {
        const columns = [
            ...
            {
                title: '收款码',
                dataIndex: 'collectionqrcode',
                key: 'collectionqrcode',
                render: text => <Image
                    width={48}
                    height={48}
                    src={text}
            ...
            },
            {
                ...
                dataIndex: 'enable',
                key: 'enable',
                render: (text, record) => <Switch size="middle" checkedChildren="开启" unCheckedChildren="关闭" defaultChecked={text === 1 ? true : false} onChange={value => this.updatestatus([record.id])} />,
            },
            {
                title: '操作',
                key: 'id',
