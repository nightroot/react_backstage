...
import { tenantmenu } from '@/routes/tenant'
...
class IndexComponent extends Component {
    ...
    componentDidMount() {
        // 挂载 - 检查状态
        this.setState({ userinfo: { ...this.props.tenantuser } })
    }
    ...
    handleUserMenuClick = (e) => {
        ...
        if (e.key === "logout") {
            post('/tenant/logout', {
                auth:this.props.tenantuser.token,
                mobile:this.props.tenantuser.mobile
            }).then((res => {
                this.props.settenantuser({})//注销
                ...
                <Header style={{ color: 'white', zIndex: 1, width: '100%', padding: 0 }}>
                    <Row wrap={false} justify="space-between">
                        <Col flex="auto" style={{ display: 'flex', alignItems: 'center', marginLeft: 20 }}>
                            ...
                            />
                            <Title style={{ color: 'white', display: 'inline-block', marginTop: 6 }} level={3}>安居房屋租赁租客管理系统</Title>
                        </Col>
                ...
                <Layout>
                    ...
                    >
                        <Menu theme="dark" defaultSelectedKeys={['1']} mode="inline">
                            {tenantmenu.map(parents => {
                                if (parents.isShow === false) { return {} }
                                if (parents.children === undefined) {
                                    return (
                                        <Menu.Item key={parents.path} onClick={p => this.props.history.push(`/tenant${p.key}`)}>
                                    ...
                                    return (
                                        <SubMenu key={parents.path} title={
                                            ...
                                            {parents.children.map((child) => {
                                                return (
                                                    <Menu.Item key={child.path} onClick={p => this.props.history.push(`/tenant${p.key}`)}>
